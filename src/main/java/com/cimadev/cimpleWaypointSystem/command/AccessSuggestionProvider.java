...
public class AccessSuggestionProvider implements SuggestionProvider<ServerCommandSource> {
    ...
    public AccessSuggestionProvider() { this(true); }
    public AccessSuggestionProvider(boolean includeOpen) {
    ...
    public CompletableFuture<Suggestions> getSuggestions(CommandContext<ServerCommandSource> context, SuggestionsBuilder builder) throws CommandSyntaxException {
        for (AccessLevel access : AccessLevel.values() ) {
            if (access == AccessLevel.OPEN && (!includeOpen || !context.getSource().hasPermissionLevel(3)) )
                continue;
